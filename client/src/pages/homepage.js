...
const Homepage = () => {
      ...
      if (loaiTaiKhoan === "Chủ trọ" && MaTK) {
        ...
        try {
          ...
          if (houses.length > 0) {
            ...
            console.log(
              "ID property extracted from first house:",
              firstHouseId
            ); // See what ID value you got
            ...
            if (firstHouseId !== null && firstHouseId !== undefined) {
              ...
              console.warn(
                "First house found, but its MaNhaTro is missing or undefined."
              );
  ...
  console.log(
    "Homepage RENDER - current selectedHouseId STATE:",
    selectedHouseId
  ); // <-- ADD THIS LOG
          ...
          >
            ...
            >
              ...
              {loaiTaiKhoan === "Chủ trọ" && (
                ...
                >
                  {loadingHouses && (
                    <p style={{ color: "grey" }}>Đang tải danh sách nhà...</p>
                  )}
                  {!loadingHouses && rentalHouses.length === 0 && (
                    <p style={{ color: "grey" }}>Không tìm thấy nhà trọ nào.</p>
                  )}
                  {!loadingHouses &&
                    rentalHouses.map((house) => (
                      ...
                      <button
                        key={house.MaNhaTro}
                        className={`address-btn btn-2 ${
                          selectedHouseId === house.MaNhaTro ? "active-btn" : ""
                        }`}
                        onClick={() => handleHouseSelect(house.MaNhaTro)}
                      >{`${
